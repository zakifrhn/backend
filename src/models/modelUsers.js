...
model.getByUser = (username) => {
  return new Promise((resolve, reject) => {
    db.query(
      "SELECT id_user, email, roles, fullname, phone_number, image_user FROM public.users WHERE username = $1",
      [username]
    )
      .then((res) => {
        resolve(res.rows);
...
model.getByPassword = (username) => {
  return new Promise((resolve, reject) => {
    db.query("SELECT pass, roles from public.users WHERE username = $1", [
      username,
    ])
      .then((res) => {
        if (res.rows.length) {
